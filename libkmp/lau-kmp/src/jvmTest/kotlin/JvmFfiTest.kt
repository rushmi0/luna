...
class JvmFfiTest {
    ...
    @Test
    fun jvm_exec_returns_true_on_success() {
        makeVm().use { vm ->
            val ok = vm.exec("x = 1")
            assertTrue(ok)
        }
    ...
    @Test
    fun jvm_run_returns_boolean_true() {
        makeVm().use { vm ->
    ...
    @Test
    fun jvm_run_returns_boolean_false() {
        makeVm().use { vm ->
    ...
    @Test
    fun jvm_set_get_global_boolean() {
        makeVm().use { vm ->
    ...
    @Test
    fun jvm_syntax_error_throws_Syntax() {
        makeVm().use { vm ->
    ...
    @Test
    fun jvm_runtime_error_throws_Runtime() {
        makeVm().use { vm ->
    ...
    @Test
    fun jvm_lua54_integer_division() {
        makeVm().use { vm ->
    ...
    // -- runFile (JVM only: uses java.io.File for temp file) --

    @Test
    fun jvm_run_file_executes_script() {
        val tmp = java.io.File.createTempFile("luna_test", ".lua")
        try {
            tmp.writeText("result = 'from_file'")
            makeVm().use { vm ->
                vm.runFile(tmp.absolutePath)
                val v = vm.getGlobal("result")
                assertIs<LocalValue.LuaString>(v)
                assertEquals("from_file", v.v1)
            }
        } finally {
            tmp.delete()
        }
    ...
    @Test
    fun jvm_run_file_missing_path_throws_Other() {
        makeVm().use { vm ->
            assertFailsWith<LuaException.Other> {
                vm.runFile("/nonexistent/path/script.lua")
            }
        }
    }
